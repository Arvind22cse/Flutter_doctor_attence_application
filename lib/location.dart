...
class _LocationTrackerState extends State<LocationTracker> {
  ...
  Future<void> getCurrentLocation() async {
    ...
    try {
      ...
      final response = await http.get(
        ...
        headers: {'User-Agent': 'com.example.my_app (arvindm.22cse@kongu.edu)'},
      );
    ...
    if (locationText.startsWith("Your current location:")) {
      ...
      try {
        final response = await http.post(
          Uri.parse('http://192.168.178.19:3002/api/location'),
  ...
  Future<void> markAttendance() async {
    ...
    if (locationText.startsWith("Your current location:")) {
      ...
      try {
        final response = await http.post(
          Uri.parse('http://192.168.178.19:3002/api/attendance'),
          headers: {'Content-Type': 'application/json'},
          body: jsonEncode({'doctorId': doctorId, 'address': locationText}),
        );
  ...
  Future<void> markCheckOut() async {
    ...
    if (locationText.startsWith("Your current location:")) {
      ...
      try {
        final response = await http.post(
          Uri.parse(
            'http://192.168.178.19:3002/api/checkout',
